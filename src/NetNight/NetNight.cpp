...
NetFight::NetFight() {
    ...
    // SDL Net starten
    if(SDLNet_Init()==-1) {
        printf( "SDL Net error: %s", SDLNet_GetError());
        SDL_Quit();
        return;
    }
    // Grundwert
...
int NetFight::AddClient( std::string Name, int ID, TCPsocket Sock) {
    if( isNameValid( Name) ) {
        ...
        printf( "Username '%s'hat falsche zeichen drin(%d)", Name.c_str(), ID);
        SDLNet_TCP_Close( Sock);
        return -1;
    }
    if( CheckClient( Name) == true ) { // Check ob es vorhanden ist
        SendMsg( Sock, "User schon vorhanden. Anderen Name bitte wahlen");
        printf( "Konnte Nutzer '%s' nicht erstellen, schon vorhanden(%d)", Name.c_str(), ID);
        SDLNet_TCP_Close( Sock);
        return -1;
    }
    ...
    printf( "Client '%s' wurde hinzugefugt(%d)", Name.c_str(), ID);
    return ID;
}
...
int NetFight::DeleteClient( std::string Name) {
    if( CheckClient( Name) == false ) { // check
        printf( "Client mit dem Name '%s' nicht gefunden", Name.c_str());
        return 0;
    }
    int ID;
    for( int i = 0; i < (int)Clients.size(); i++)  {// suche nach gleichen namen
        if( Clients[i].Name == Name) {
            ...
            printf( "User '%s' wurde erfolgreich geloscht(%d)", Clients[i].Name.c_str(), ID);
...
int NetFight::Connect( std::string sIP, int Port) {
    ...
    if(!SocketSet) {
		printf("SDLNet_AllocSocketSet: %s", SDLNet_GetError());
		return -1;
	}
	...
	if(SDLNet_TCP_AddSocket( SocketSet, ServerSocket)==-1) {
		printf("SDLNet_TCP_AddSocket: %s",SDLNet_GetError());
		return -1;
	}
...
int NetFight::SendMsg( TCPsocket Socket, std::string Msg) {
	...
	if( Result < (int)sizeof( Length )) {
		if( SDLNet_GetError() && strlen(SDLNet_GetError())) // meinchmal leer
			printf("SDLNet_TCP_Send: %s", SDLNet_GetError());
		return 0;
	}
...
std::string NetFight::GetMsg(TCPsocket Socket) {
	...
	if( Result < (int)sizeof( Length)) {
		if(SDLNet_GetError() && strlen(SDLNet_GetError())) // Meinchmal leer
			printf("SDLNet_TCP_Recv: %s", SDLNet_GetError());
		return Buffer;
	}

...
void Control::Handle( Input *input, EntityList *entity) {
    ...
    if( input->Map.Place && !input->MapOld.Place) {
        Entity *t_ent = NULL;
        t_ent = entity->findEntityMarked( entity->getTree());
        if( t_ent == NULL)
            return;
        if( t_ent->GetCellName() == collector)
            entity->createTeamEntity( collector,  input->Map.MousePos.m_x, input->Map.MousePos.m_y, t_ent);
        else
            entity->editData( 1, input->Map.MousePos.m_x, input->Map.MousePos.m_y, t_ent); //entity->editData( );
        //m_entity->createEntity( cell, m_input.Map.MousePos.m_x, m_input.Map.MousePos.m_y, yellow);
        //printf( "m_number %d\n", t_ent->getNumber() );
    }
}
